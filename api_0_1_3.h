...
extern "C"
{
	...
	*/
	...
	/*
	 verification
	*/
	//suggestion that this is good point to verify
	int verify_optional_suggest (	MPC_CTX * ctx,
									int * error);  //error is out parameter. 0 if no error, 
									//-1 if verify failed and should abort

	//the verify function must verify all pending outputs not verified yet
	int verify_final			 (	MPC_CTX * ctx,
									int * error);  //error is out parameter. 0 if no error, 
									//-1 if verify failed and should abort
